...
export async function addVacationDocument(body: AddVacationRequest) {
  return await api
    .post('/api/document/vacation', { json: body })
    .json<Documents>();
}
...
export async function getVacationDocument(id: number) {
  return await api
    .get(`/api/document/vacation/${id}`)
    .json<VacationDocumentDetail>();
}
export async function approvalDocument(params: PageParams) {
  return await api
    .get('/api/document/approval/search', { searchParams: params })
    .json<Page<DocumentApproval>>();
}

export async function getApproveDocument(approvalId: number) {
  return await api
    .get(`/api/document/approval/${approvalId}`)
    .json<DocumentApproval>();
}
...
export async function approveDocument(
  ...
  return await api
    .put(`/api/document/approval/${id}`, { json: body })
    .json<DocumentApproval>();
}

export async function addHolidayWorkReport(body: AddHolidayWorkReportRequest) {
  return await api
    .post('/api/document/holiday', { json: body })
    .json<HolidayWorkReport>();
}

export async function getHolidayWorkReports(documentId: number) {
  return await api
    .get(`/api/document/holiday/${documentId}`)
    .json<HolidayWorkReportDetail>();
}

export async function getVacation(params: {
  startDate: string;
  endDate: string;
}) {
  return await api
