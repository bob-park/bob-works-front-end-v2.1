...
export async function getChatRooms(params: SearchChatRoomRequest) {
  return await api.get('/api/chat/room/all', { searchParams: params }).json<ChatRoomResponse[]>();
}
...
export async function addChatRoomUsers(roomId: number, body: AddChatRoomUserRequest) {
  return await api.post(`/api/chat/room/${roomId}/users`, { json: body }).json<ChatRoomUserResponse[]>();
}
